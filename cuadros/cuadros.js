...
for (let i = 0; i < 4; i++){//arriba
	...
	scene.add(wireC);
	*/
var txt1 = new THREE.TextureLoader().load(`cuadros/${i+5}.jpg`);
...
cdrB1.position.set(-3+i*2,0,-3.725);
...
//borde cuadro 1
var cdrB1 = new THREE.Mesh(cuadroBG, cuadroBM);
cdr1.rotation.y = Math.PI/2;
cdrB1.rotation.y = Math.PI/2;
cdr1.position.set( -4.37+0.025 ,0,-2.7+i*1.8 );
cdrB1.position.set(-4.37,0,-2.7+i*1.8);
scene.add(cdr1,cdrB1);
}
for (let i = 0; i < 4; i++){//der
	...
	scene.add(wireC);
	*/
var txt1 = new THREE.TextureLoader().load(`cuadros/${i+9}.jpg`);
var cuadroMat =  new THREE.MeshBasicMaterial({color:0xffffff,map:txt1});
//cuadro1
var cdr1 = new THREE.Mesh(cuadroGeo, cuadroMat);
//borde cuadro 1
